...
> **The canonical, current parity report is [`../BENCHMARKS.md`](../BENCHMARKS.md)**
> — fresh Apple-M4-Max numbers vs **native FFTW**, numpy, scipy and gonum, with
> GFLOP/s and per-op action items, regenerated by `benchmarks/run.sh`. This page
> is the optimization-history narrative (before/after deltas from each round,
> originally measured on a 4-core arm64 Linux host) and is kept for that context.

## How to reproduce

The full standardized sweep lives in the self-contained `benchmarks/` module
(so the library `go.mod` stays dependency-free — `gonum` is never pulled in) and
is driven by one script:

```sh
benchmarks/run.sh            # correctness gate + go-fft/gonum + native FFTW + numpy/scipy → BENCHMARKS.md
```

It runs, on the same host and the same inputs: the Go matrix
(`benchmarks/parity_test.go`, go-fft via its cached `Plan` API + gonum), the
native-FFTW C harness (`benchmarks/cbench/fftw_bench.c`), and the
numpy.fft/scipy.fft Python harness (`benchmarks/fft_reference.py`) — all
single-threaded, plan-reused, best-of-N.
